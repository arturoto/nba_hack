...
df_viewershipmodel = df[['Total Viewership','Network','Total Households','Total_All_Stars','Holiday','Weekend','Total_Top_Jersey_Sellers','Past_Playoff_Teams','Rivalry','avg_pace','Higher_Elo','Lower_Elo']]
...
#df_attendancemodel = df[['Percent_Tickets_Sold','Home_All_Star','Away_All_Star','Holiday','Weekend','Home_Top_Jersey_Sellers','Away_Top_Jersey_Sellers','Past_Playoff_Teams','Rivalry','Home_Pace_Rating','Away_Pace_Rating','Home_Elo','Away_Elo']]
#df_attendance = pd.read_csv('C:/Users/Nick.Ceneviva/Documents/hackathon/League_Entertainment_Value/League_Entertainment_Value/Pct_Indy_Tix_Sold.csv',header = 0)
#
#df_attendance['Days until game'] = pd.to_datetime(df_attendance['Game_Date'])-pd.to_datetime(df_attendance['Current_Date'])
#
#df_attendance[df_attendance['Days until game'] == '15 Days 00:00:00']
#df_attendance.rename(columns={'Game_Date': 'Date'},inplace = True)
df['SellOut'] = (df['Percent_Tickets_Sold']==1)

#df_attendancedata = pd.merge(df, df_attendance, how='inner', on=['Date', 'Away'])
